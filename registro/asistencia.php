...
// Si no hay evento, tomar el de hoy
if ($evento_id_sel === 0) {
    $hoy = date('Y-m-d');
    $stmt = $conexion->prepare("SELECT id FROM eventos WHERE activo=1 AND fecha=? ORDER BY hora_inicio ASC LIMIT 1");
    $stmt->bind_param("s", $hoy);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    if ($row) $evento_id_sel = (int)$row['id'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($evento_id <= 0) {
        ...
        $stmtA->execute();
        $as = $stmtA->get_result()->fetch_assoc();

        if (!$as) {

    $toast = [
        'type'=>'err',
        'msg'=>'❌ DNI '.$codigo.' no encontrado'
    ];

} elseif (strtoupper($as['deuda']) === 'SI') {

    $toast = [
        'type'=>'err',
        'msg'=>'🚫 '.$as['nombre'].' (DNI '.$as['dni'].') TIENE DEUDA'
    ];
    ...
    try {
        ...
        $toast = [
            'type'=>'ok',
            'msg'=>'✅ '.$as['nombre'].' (DNI '.$as['dni'].') REGISTRADO'
        ];
        ...
        if ($e->getCode() === 1062) {
            ...
            $toast = [
                'type'=>'err',
                'msg'=>'⚠️ '.$as['nombre'].' (DNI '.$as['dni'].') YA REGISTRADO'
            ];
...
// Eventos
$evtList = $conexion->query("SELECT id,titulo,fecha FROM eventos WHERE activo=1 ORDER BY fecha DESC");
...
<div class="grid">
  ...
  <div class="card col-5">
      ...
      <div style="flex:1">
        ...
        <select name="evento_id" onchange="this.form.submit()">
          ...
          <?php while($e = $evtList->fetch_assoc()): ?>
            <option value="<?= $e['id'] ?>" <?= $e['id']==$evento_id_sel?'selected':'' ?>>
              <?= $e['fecha'].' '.$e['titulo'] ?>
            </option>
          <?php endwhile; ?>
        </select>
    ...
    <form method="post" class="form">
      ...
      <div class="col-6">
        <label>Hora</label>
        <input class="input" value="<?= substr($ahora,0,5) ?>" disabled>
      </div>
