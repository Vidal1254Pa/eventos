...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'crear') {
        ...
        if ($titulo && $fecha && $hora_inicio) {
            ...
            $stmt = $conexion->prepare("INSERT INTO eventos(titulo,descripcion,lugar,fecha,hora_inicio,hora_fin,activo,creado_por)
                VALUES(?,?,?,?,?,?,?,?)");
            $stmt->bind_param("ssssssii", $titulo,$descripcion,$lugar,$fecha,$hora_inicio,$hora_fin,$activo,$uid);
            $stmt->execute();
            flash_set('ok','Evento creado.');
        } else {
            flash_set('err','Complete: título, fecha y hora de inicio.');
        }
    ...
    if ($accion === 'editar') {
        ...
        } else flash_set('err','Datos inválidos.');
        redirect('admin/eventos.php');
    }
    ...
    if ($accion === 'eliminar') {
        ...
        if ($id>0) {
            ...
            $stmt->execute();
            flash_set('ok','Evento eliminado (también sus asistencias).');
        }
        redirect('admin/eventos.php');
    }
}
...
$lista = $conexion->query("SELECT e.*,
  (SELECT COUNT(*) FROM asistencias a WHERE a.evento_id=e.id) AS asistencias_count
  FROM eventos e ORDER BY e.fecha DESC, e.hora_inicio DESC");
...
<div class="grid">
  <div class="card" style="grid-column: span 5;">
    ...
    <form method="post" class="form">
      ...
      <div class="col-12"><label>Título</label><input class="input" name="titulo" required value="<?= h($edit['titulo'] ?? '') ?>"></div>
      <div class="col-12"><label>Descripción (opcional)</label><input class="input" name="descripcion" value="<?= h($edit['descripcion'] ?? '') ?>"></div>
      ...
      <div class="col-3"><label>Inicio</label><input class="input" type="time" name="hora_inicio" required value="<?= h(substr($edit['hora_inicio'] ?? '08:00:00',0,5)) ?>"></div>
      <div class="col-3"><label>Fin</label><input class="input" type="time" name="hora_fin" value="<?= h(substr($edit['hora_fin'] ?? '',0,5)) ?>"></div>

      <div class="col-12">
        <div class="row" style="align-items:center">
          <input type="checkbox" name="activo" <?= !$edit || (int)($edit['activo'] ?? 1)===1 ? 'checked' : '' ?>>
  ...
  <div class="card" style="grid-column: span 7;">
    <h2>Eventos</h2>
    <table class="table">
      <thead><tr><th>Fecha</th><th>Hora</th><th>Título</th><th>Lugar</th><th>Act.</th><th>Asist.</th><th>Acciones</th></tr></thead>
      <tbody>
        <?php while($e = $lista->fetch_assoc()): ?>
          <tr>
            <td><?= h($e['fecha']) ?></td>
            <td><?= h(substr($e['hora_inicio'],0,5)) ?><?= $e['hora_fin'] ? ' - '.h(substr($e['hora_fin'],0,5)) : '' ?></td>
            <td><?= h($e['titulo']) ?></td>
            <td><?= h($e['lugar'] ?? '') ?></td>
            <td><?= (int)$e['activo']===1 ? 'Sí' : 'No' ?></td>
